...
create_ecoc = function(task,type=c("1vsRest","1vs1","custom")[1],custom_mat=NA) {
  if (length(task$labels)!=1) {
...
train_lr = function(trainfold,task,code,code_cols=1:code$n_cols) {
  ...
  for(i in 1:length(code_cols)){
    bootstrap_rows[[i]] = sample(train_rows,replace=T);
  }

  features=foreach (i=1:length(code_cols)) %dopar% {

    
    #bootstrap_rows = sample(train_rows,replace=T);
    trainfold$rows = bootstrap_rows[[i]];  
    ...
    feature = tmp_create_feature(model)
    return(feature)
  }
