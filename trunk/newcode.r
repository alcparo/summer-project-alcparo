...
for (cv in 1:1) {
    ...
    split_folds[[paste(sp)]] = create_fold(dm,rows=rows,name=paste(sp)) #paste("cv",fold_name,sep="")) 
  	...
  	for (i2 in 1:nrow(model_types)) { #### ECOC COLUMNS
    	...
    	for(i3 in 1:ensemble_size){

    		bootstrap_rows = sample(train_rows,replace=T);
    		split_folds[['1234']]$rows = bootstrap_rows;

    		for (i4 in 1:nrow(modcalrel_splits)) {
			      name3 = paste(name2,".s",i4,sep="")
			      print(name3)

			      mod_fold = split_folds[[paste(modcalrel_splits$mod[i4])]]
			      cal_fold = split_folds[[paste(modcalrel_splits$cal[i4])]]
			      rel_fold = split_folds[[paste(modcalrel_splits$rel[i4])]]
			      if (model_types$names[i2]=='svmrad') { ##### ALGORITHM LEARNING TYPE
			        ...
			        mod = train_svm_binary(mod_fold,modtask,ecoc,i2,kernel="polynomial",degree=polydegrees[[dataset]])
			      } else if (model_types$names[i2]=='lr') {
			        mod = train_lr_binary(mod_fold,modtask,ecoc,i2)
			      ...
			      for (i5 in 1:length(mod)) {
			        # code_name = colnames(ecoc$mat)[i5]
			        code_name = colnames(ecoc$mat)[i2]
			        # name4 = paste(name3,code_name,sep=".")
			        name4 = paste(paste(name3,code_name,sep="."),".bootstrap",i3,sep="");
			      ...
			      # ggsave(paste(dataset,name3,"cal.pdf",sep="."),plot_calrel_binary(dm,name3,ecoc,cols=i2,calrel=".hcal"),width=5,height=5)
			      # ggsave(paste(dataset,name3,"rel.pdf",sep="."),plot_calrel_binary(dm,name3,ecoc,cols=i2,calrel=".hrel"),width=5,height=5)
			...
			#     for (i4 in 1:n_methods) {
			#       name3 = paste(name2,".m",i4,sep="")
			#       print(name3)
			#       splvec = which(modcalrel_splits$method==i4)
...
# dm$mat[1:5,paste("cv1.svmpol.s1.model1.bootstrap", 1:5, sep="")]
# ifelse((rowSums(dm$mat[1:dm$n,paste("cv1.svmpol.s1.model1.bootstrap", 1:5, sep="")])/ensemble_size)>0.5,1,0) 



# sum(ifelse((rowSums(dm$mat[test_fold$rows,paste("cv1.svmpol.s1.model1.bootstrap", 1:5, sep="")])/ensemble_size)>0.5,1,0)  == dm$mat[,'class'])/dm$n